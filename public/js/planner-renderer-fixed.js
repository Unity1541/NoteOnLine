// Shared planner UI renderer for both user and admin views.
...
export function renderHeader(state, userDisplayName, weekDisplay, currentTimeDisplay) {
    ...
    weekDisplay.textContent = `${weekStart.getFullYear()}/${weekStart.getMonth() + 1}/${weekStart.getDate()} - ${weekEnd.getMonth() + 1}/${weekEnd.getDate()}`;
    currentTimeDisplay.textContent = new Date().toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' });
}
...
export function renderCalendarGrid(state, calendarGrid) {
    ...
    weekDays.forEach((day, index) => {
        ...
        for (let min = 0; min < 1020; min += 5) {
            ...
            const lineClass = isHour ? 'border-stone-200' : 'border-stone-100';
        ...
        if (dayEvents.length > 0) {
            summaryHTML += `
                <div class="flex justify-between text-sm bg-white/40 rounded-xl p-2 backdrop-blur-sm">
                    ...
                    <span class="font-bold text-indigo-600">${dayEvents.length} 項</span>
                </div>
            `;
        }
    });
    ...
    if (weekEvents.length === 0) {
        checklistContent.innerHTML = `
            <div class="text-center py-8 text-slate-400">
                ...
                <p class="text-sm font-medium">本週目前沒有排程項目</p>
            </div>
        `;
        return;
    }
    ...
    weekEvents.forEach((event) => {
        ...
        item.innerHTML = `
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                ...
                <button class="export-event-pdf-btn text-slate-400 hover:text-slate-700 transition-colors duration-200 p-2 rounded-full hover:bg-white/50" title="匯出此行程 PDF" type="button">
